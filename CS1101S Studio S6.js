...
function percussions(distance, list_of_sounds, rhythm) {
    function list_of_sound_to_play(list_of_sounds,rhythm,counter) {
        return counter > length(rhythm)-1
            ? null
            : pair(list_ref(list_of_sounds,list_ref(rhythm,counter)),
                   list_of_sound_to_play(list_of_sounds,
                                                 rhythm,
                                                 counter+1));
    }
    function repeat_this_list(list_of_sounds,n,i,distance) {
        return n <= 0
            ? null
            : pair(consecutively(pair(silence_sound(distance*i),
                                      list_of_sounds)),
                   repeat_this_list(list_of_sounds,
                                               n-1,
                                               i+1,
                                               distance));
    }
    return simultaneously(repeat_this_list(list_of_sound_to_play(list_of_sounds,
                                                                         rhythm,
                                                                         0),
                                           length(rhythm),
                                           0,
